...
function get1(x, y) {
  const n = x.getElementsByTagNameNS('*', y);
...
function gxCoords(root) {
  let elems = root.getElementsByTagNameNS('*', "coord");
  const coords = [];
  const times = [];
  if (elems.length === 0) elems = root.getElementsByTagNameNS('*', "gx:coord");
  ...
  const timeElems = root.getElementsByTagNameNS('*', "when");
...
function getGeometry(root) {
  ...
  for (i = 0; i < geotypes.length; i++) {
    geomNodes = root.getElementsByTagNameNS('*', geotypes[i]);
    if (geomNodes) {
      for (j = 0; j < geomNodes.length; j++) {
        ...
        if (geotypes[i] === "Point") {
          ...
          const rings = geomNode.getElementsByTagNameNS('*', "LinearRing"),
            coords = [];
...
function getPlacemark(root, styleIndex, styleMapIndex, styleByHash) {
  ...
  if (extendedData) {
    const datas = extendedData.getElementsByTagNameNS('*', "Data"),
      simpleDatas = extendedData.getElementsByTagNameNS('*', "SimpleData");
...
export function* kmlGen(doc) {
  ...
  const styleMaps = doc.getElementsByTagNameNS('*', "StyleMap");
  ...
  for (let l = 0; l < styleMaps.length; l++) {
    ...
    const pairs = styleMaps[l].getElementsByTagNameNS('*', "Pair");
    const pairsMap = {};
